...
$(window).scroll(function(){
    ...
    // var index = $(".zm_floor").index();
    // var itemtop = $(".zm_floor").eq(index).offset().top;
    var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
    var height = document.documentElement.clientHeight || document.body.clientHeight;
    var floor = $(".zm_floor");
    var num;
    for (var i=0; i<floor.length; i++){
        var floor_top = $(".zm_floor").eq(i).offset().top;
        // if(scrollTop<floor_top && scrollTop+height>floor_top){
        //     num = i;
        // }
        if(scrollTop>floor_top - 80){
            num = i;
        }
        $(".bar_cell").eq(num).addClass("width_bar").siblings().removeClass("width_bar");
    }


});

$(".bar_cell").click(function(){
    var index = $(this).index();
    var floor_top = $(".zm_floor").eq(index).offset().top;
    // console.log(floor_top)
    // $("body").scrollTop(floor_top);
    // $("html").scrollTop(floor_top);
    // var _self = $(this);
    $("body").animate({
        scrollTop: floor_top
    });
...
$('.btm_btn').click(function(){
    // console.log($('.zm_apply').parents(".zm_floor").offset().top)
    $('html').animate({
            scrollTop:$('.zm_apply').parents(".zm_floor").offset().top
        },
        1000);
    $('body').animate({
            scrollTop:$('.zm_apply').parents(".zm_floor").offset().top
...
$(".sub_btn").click(function(){
    var user_name = $("#user_name").val();   //联系人
    var user_tel = $("#user_tel").val();    //联系电话
    var user_company = $("#user_company").val();  //企业名称
    var company_type = $("#company_type").val();  //经营类型
    var user_Province = $("#s_province").val();   //所在省份
    var user_City = $("#s_city").val();   //所在城市
    var user_County = $("#s_county").val();   //所在区
    var user_addr = $("#user_addr").val();  //所在街道
    var user_note = $("#user_note").val();  //备注
    const  celltype = $(".list_cell input[type='radio']");
    var celltype_val = '';  //选中值
    $(".list_cell input[type='radio']").each(function(i, item){
        if (celltype[i].checked){
            celltype_val = $(this).val()
        }
    })
    $(".zm_apply_item input[type='text'],.zm_apply_item input[type='tel']").each(function(i, item){
        $(this).focus(function(){
            $(this).next(".phone").hide();
            $(this).next(".error_message").hide();
        })
    })
    // var myreg = /^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/;
    var myreg = /^(13[0123456789][0-9]{8}|15[012356789][0-9]{8}|18[0123456789][0-9]{8}|147[0-9]{8}|1349[0-9]{7}|17[0123456789][0-9]{8})$/;
    if(user_name == ''|| user_name==undefined){
        $("#user_name").next(".error_message").show();
        return false;
    }else if(!myreg.test(user_tel)) {
        $(".phone").show();
        return false;
    }else if(user_company== ''|| user_company==undefined){
        $("#user_company").next(".error_message").show();
        return false;
    }else if(company_type== ''|| company_type==undefined){
        $("#company_type").next(".error_message").show();
        return false;
    }else {
        var url = Url+"city_partner.php?act=brand_supplier";
        var data = {
            "user_name"        : user_name,
            "tel"              : user_tel,
