function MoveImg() {
	$(document).ready(function() {
		...
		interval = 85
		var curIndex = 0;
		var imgLenght = $("#imageMenu li").length;

		if(imgLenght > 4){
			$('.scrollbutton').click(function() {
				if($(this).hasClass('disabled')) return false;

				if($(this).hasClass('midImgUp')) --curIndex;
				else ++curIndex;

				$('.scrollbutton').removeClass('disabled');
				if(curIndex == 0) $('.midImgUp').addClass('disabled');
				if(curIndex == count - 1) $('.midImgDown').addClass('disabled');
		...
		$("#imageMenu li img").bind("mouseover", function() {
			if($(this).attr("id") != "onlickImg") {
				midChange($(this).attr("src").replace("mid", "mid"));
			...
			if($(this).attr("id") != "onlickImg") {
				...
				midChangeHandler = window.setTimeout(function() {
					midChange($("#onlickImg img").attr("src").replace("mid", "mid"));
				}, 1000);
			}
		});
